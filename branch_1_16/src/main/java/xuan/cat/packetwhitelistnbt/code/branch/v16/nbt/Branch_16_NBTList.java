...
public final class Branch_16_NBTList extends BranchNBTList {
    protected NBTTagList tag;

    public Branch_16_NBTList() {
        this.tag = new NBTTagList();
    }

    public Branch_16_NBTList(NBTTagList tag) {
        this.tag = tag;
    }
    ...
    public BranchNBTType getOwnType() {
        return Branch_16_NBTType.fromNMS(tag.d_());
    }
    ...
    public void add(int index, Object value) {
        if (value instanceof Branch_16_NBTCompound) {
            tag.add(index, ((Branch_16_NBTCompound) value).getNMSTag());
        } else if (value instanceof Branch_16_NBTList) {
            tag.add(index, ((Branch_16_NBTList) value).getNMSTag());
        } else {
